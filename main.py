...
def generate_map(cfg: dict):
    ...
    # Unified map-wide texture scheme — all rooms and bridges share one triple
    map_floor_t = random.choice(FLOOR_TEX)
    map_wall_t  = random.choice(WALL_TEX)
    map_ceil_t  = random.choice(CEIL_TEX)
    for r in rooms:
        r.floor_t, r.wall_t, r.ceil_t = map_floor_t, map_wall_t, map_ceil_t
    for br in bridges:
        br.floor_t, br.wall_t, br.ceil_t = map_floor_t, map_wall_t, map_ceil_t
    ...
    # --- start trigger — thin slab (line) the player crosses, 128 u ahead of spawn
    TRIG_T = 8   # trigger thickness in units
    if first.travel_axis == 'x':
        trig_cx = first.x1 + 192   # 128 u ahead of spawn at x1+64
        trig_x1, trig_x2 = trig_cx - TRIG_T // 2, trig_cx + TRIG_T // 2
        trig_y1, trig_y2 = first.y1, first.y2
    else:
        trig_cy = first.y1 + 192
        trig_x1, trig_x2 = first.x1, first.x2
        trig_y1, trig_y2 = trig_cy - TRIG_T // 2, trig_cy + TRIG_T // 2
    lines.append(f"\n// entity {ei}")
    lines.append(ent_brush_box("trigger_multiple",
        trig_x1, trig_y1, first.z1,
        trig_x2, trig_y2, first.z1 + DOOR_H,
        target="start_t"))
    ei += 1
